...
- Java 25, Fabric Loom 1.17 (`net.fabricmc.fabric-loom` plugin id), Mojang official mappings — Minecraft 26.1+ ships unobfuscated, so there is no mapping file at all; class/method names in this codebase are Mojang's own ("Mojmap": `ServerPlayer`, `CompoundTag`, `Component`, etc.), not Yarn's
