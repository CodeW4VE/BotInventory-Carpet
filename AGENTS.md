...
Unlike later branches, armor and offhand **are** part of `"Inventory"` here — 1.21 predates the separate `"equipment"` NBT key (added ~1.21.5), so `PlayerInventory#writeNbt`/`readNbt` already encode all 41 slots (36 main+hotbar, 4 armor, 1 offhand) into the one list, keyed by slot index. `EDITABLE_KEYS` is just `"Inventory"` and `"EnderItems"` here; no separate equipment handling is needed.
