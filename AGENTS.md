...
Inventory redirects only cover indices `0..DISPLAYED_INVENTORY_SIZE-1` (41: 36 main+hotbar, 4 armor, 1 offhand), never `PlayerInventory#size()` (43 in 1.21.11) directly — `size()` also counts the `BODY`/`SADDLE` equipment slots added for other entity types, which exist in every player's slot map but aren't meaningful for a player and shouldn't be shown as editable.
