...
public class ViewCommand {
     ...
     * PlayerInventory#size() (43), which also counts non-editable
    ...
    public static boolean isViewAllowed(ServerCommandSource source, String ruleValue) {
        return checkRule(ruleValue, source::hasPermissionLevel);
    }
    ...
    private static Resolved resolveTarget(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        if (!ViewSessions.tryRegisterOffline(target.entry().getId(), session)) {
        ...
        try {
            ...
            ViewSessions.unregisterOffline(target.entry().getId(), session);
            throw e;
        }
